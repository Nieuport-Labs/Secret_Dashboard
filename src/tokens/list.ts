/**
 * The built-in token list, as plain data.
 *
 * Split out of `registry.ts` so it imports nothing: `middleware.ts` names the
 * token on an invoice's link preview from this list, and runs as native ESM
 * outside Vite, where neither the `@/` alias nor the zustand store the
 * registry reads custom tokens from exist. Everything else should keep going
 * through `registry.ts`.
 */
