...
/*
 * The original module carried its own code-hash cache. This project already has
 * one in `@/lib/codeHash`, which is the single place the app obtains a hash, so
 * the local copy is dropped rather than kept alongside it: two caches over the
 * same chain fact drift, and a stale hash does not degrade — it stops every
 * query on the contract dead.
 */
