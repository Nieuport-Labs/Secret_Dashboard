 ...
 *
 * Plain CommonJS `.cjs`, not TypeScript — this took four attempts to land on:
 *
 *   .tsx (default export)   → compiled to a `.js` carrying a literal `import`
 *                              with no `"type": "module"` Node could find —
 *                              "Cannot use import statement outside a module".
 *   .tsx (named GET export)  → identical failure; the export shape was never
 *                              the problem.
 *   .mts (no JSX)             → not a Function entrypoint extension Vercel's
 *                              zero-config builder recognises at all — it
 *                              built nothing, and requests silently fell
 *                              through to the SPA's catch-all rewrite.
 *   .ts (no JSX)              → finally built and ran as a real ES module —
 *                              and then failed *inside* `@vercel/og` itself:
 *                              its Node build's CJS-interop shim calls a
 *                              dynamic `require("fs")` to load harfbuzzjs,
 *                              which Node refuses under `import`. The package
 *                              is built to be `require()`d, not `import`ed.
 *
 * `.cjs` is unambiguous the other direction from `.mts` — always CommonJS,
 * regardless of any `"type"` field — which is what finally lines up with how
 * `@vercel/og`'s own Node build expects to be loaded. No JSX either way, so
 * the element tree is built with a small `h(type, props, ...children)`
 * helper instead of markup — the same `{ type, props }` shape JSX compiles to
 * is all `ImageResponse` (via satori) ever needed.
 *
 ...
 * if either ever drifts. Being plain JS, this file also sits outside the
 * project's own `tsc` typecheck — a deliberate trade for a build that
 * actually deploys; keep it small and re-read it in full on any change.
 */
...
function shorten(address, head, tail) {
